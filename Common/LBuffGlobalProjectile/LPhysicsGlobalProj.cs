...
{
    public class LPhysicsGlobalProj : GlobalProjectile
    {
        ...
        {
            if ((proj == null || !proj.active) && LPhysicsSystem.magneticEntity.Contains(proj))
            {
                LPhysicsSystem.magneticEntity.Remove(proj);
            }

            if (!LPhysicsSystem.magneticEntity.Contains(proj) && (canBeAttractedByMagnet || hasMagnetism))
            {
                LPhysicsSystem.magneticEntity.Add(proj);
            }
            if (LPhysicsSystem.magneticEntity.Contains(proj) && !canBeAttractedByMagnet && !hasMagnetism)
            {
                LPhysicsSystem.magneticEntity.Remove(proj);
